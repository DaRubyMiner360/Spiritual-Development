...
public class Server implements Runnable {
	...
	private List<Client> clients2 = new ArrayList<Client>();
	...
	public void run() {
		...
		while (running) {
			...
			if (text.equals("raw")) {
				...
				String name = text.split(" ")[1];
				String stringStatus = text.split(" ")[2];
				boolean status = true;
				if(stringStatus == "true")
					status = true;
				else if(stringStatus == "false")
					status = false;
				else System.out.println("Invalid Second Argument: " + stringStatus + "! Must be 'true' or 'false'!");
				...
				if (number) {
					...
					if (exists) makeAdmin(id, status);
					else System.out.println("Client " + id + " doesn't exist! Check ID number.");
				} else {
					for (int i = 0; i < clients.size(); i++) {
						ServerClient c = clients.get(i);
						if (name.equals(c.name)) {
							makeAdmin(c.getID(), status);
							break;
						}
					}
				}
	...
	private void printHelp() {
		...
		System.out.println("/admin [users ID or username] - makes a user an ADMIN.");
	}
	...
	private void makeAdmin(int id, boolean status) {
		ServerClient c = null;
		Client client = null;
		boolean existed = false;
		for (int i = 0; i < clients.size(); i++) {
			if (clients.get(i).getID() == id) {
				c = clients.get(i);
				client = clients2.get(i);
				client.setIsAdmin(status);
				existed = true;
				break;
			}
		}
		...
		String message = "";
		if (status) {
			message = "Client " + c.name + " (" + c.getID() + ") @ " + c.address.toString() + ":" + c.port + " is now ADMIN.";
		} else {
			message = "Client " + c.name + " (" + c.getID() + ") @ " + c.address.toString() + ":" + c.port + " is no longer ADMIN.";
		}
		System.out.println(message);
	}
